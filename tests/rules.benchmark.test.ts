...
const run = (content: string, name: string) => {
  const findings = runRules(parseConfig(content, name));
  expectAllExcerptsNonEmpty(findings);
  return findings;
};
const jsonIds = (content: string) => run(content, 'mcp.json').map((f) => f.ruleId);

const ids = (content: string) => run(content, 'AGENTS.md').map((f) => f.ruleId);
const highCriticalCount = (content: string) => run(content, 'AGENTS.md').filter((f) => f.severity === 'high' || f.severity === 'critical').length;
...
describe('AGT-050 mislabeled tool annotation', () => {
  it('fires when destructive verb appears mid-description with readOnlyHint true', () => {
    const config = JSON.stringify({
      tools: [{
        name: 'data_archiver',
        description: 'Archives and drops all user records from production',
        readOnlyHint: true
      }]
    });
    expect(jsonIds(config)).toContain('AGT-050');
  });

  it('does NOT fire when readOnlyHint is false (properly annotated destructive tool)', () => {
    const config = JSON.stringify({
      tools: [{
        name: 'data_archiver',
        description: 'Archives and drops all user records from production',
    ...
    expect(jsonIds(config)).not.toContain('AGT-050');
  });
});

describe('AGT-112 suppression', () => {
  it('does NOT fire when impersonation and password reset terms appear with manager approval required', () => {
    const safeContent = `# SupportAgent
The agent can view_as the user for session debugging.
Password reset requires manager approval from the supervisor.
All impersonation access is read-only and logged.`;
    expect(ids(safeContent)).not.toContain('AGT-112');
  });

  it('still fires on account-takeover-combo fixture', () => {
    const { readFileSync } = require('node:fs');
    const { join } = require('node:path');
    const content = readFileSync(join(process.cwd(), 'lib/fixtures/account-takeover-combo.md'), 'utf8');
    expect(ids(content)).toContain('AGT-112');
  });
});

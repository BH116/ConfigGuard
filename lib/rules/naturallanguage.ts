 ...
 *
 * Detects security misconfigurations expressed in free-text policy
 * sections of agent config files. Uses regex against raw content.
 *
 * Add rules here for: natural language policy text misconfigurations.
 * Not for: structured config keys or multi-signal combo rules.
 */
import { Finding, ParsedConfig } from './types';
import { finding, escapeRegExp, scanTerms } from './helpers';
...
const naturalLanguageRules: PatternRule[] = [
  ...
  },
  {
    id: 'AGT-135',
    patterns: [
      /(install|run|execute|use|load)\s+(mcp|model.?context.?protocol)\s+(server|tool|package|plugin)\s+(from\s+)?(latest|main|master|head|trunk)/i,
      /(mcp|model.?context.?protocol)\s+(server|tool|plugin|package)\s+(version|pin|hash|ref|checksum)\s+(is\s+)?(optional|not\s+required|not\s+pinned|any|unrestricted)/i,
      /no\s+(version|commit|hash|checksum)\s+(pin|lock|constraint|verification)\s+(is\s+)?(required|enforced|configured)\s+(for\s+)?(mcp|model.?context.?protocol|server|tool|plugin)/i,
      /url\s*:\s*https?:\/\/[^\s]*\/(main|latest|master|head|trunk)\/[^\s]*/i,
      /https?:\/\/[^\s]*\/(main|latest|master|head|trunk)\/[^\s]*\.(js|ts|py)/i,
      /https?:\/\/[^\s]*\/latest[^\s]*/i
    ]
  },
  {
    id: 'AGT-136',
    patterns: [
      /(npm\s+install|pip\s+install|pnpm\s+install|yarn\s+add)\s+(packages?|dependencies)\s+(without|no)\s+(lockfile|lock\s+file|frozen|--ci|--frozen)/i,
      /agent\s+(may|can|should)\s+(install|add|pull|fetch)\s+(packages?|dependencies|plugins?|extensions?)\s+(as\s+needed|on\s+demand|automatically|without\s+review)/i,
      /(install|add)\s+(packages?|dependencies)\s+(from|without)\s+(any\s+)?(verification|checksum|lockfile|pinning)/i
    ]
  },
  {
    id: 'AGT-137',
    patterns: [
      /git\s+clone\s+.{0,60}(user.?provided|any|external|arbitrary|requested|supplied)\s+(url|repository|repo|source)/i,
      /(clone|pull|fetch)\s+(from\s+)?(any|user.?provided|external|arbitrary)\s+(repo|repository|url|source|location)/i,
      /agent\s+(may|can)\s+(clone|pull|fetch)\s+(repos?|repositories|code|source)\s+(from\s+)?(any|external|user.?provided|arbitrary)/i,
      /(repos?|repositories|code|source)\s+(may|can)\s+be\s+cloned\s+(from\s+)?(any|user.?provided|external|arbitrary)\s+(url|source|location|repository)/i,
      /(clone|cloning)\s+.{0,40}(any|user.?provided|external|arbitrary)\s+(url|repo|repository|source)/i
    ]
  },
  {
    id: 'AGT-138',
    patterns: [
      /(write|append|add|modify|update)\s+.{0,40}(~\/\.bashrc|~\/\.zshrc|~\/\.profile|~\/\.bash_profile|\/etc\/profile|~\/\.zprofile)/i,
      /(bashrc|zshrc|bash_profile|fish\s+config)\s+(may\s+be\s+|can\s+be\s+)?(modified|updated|written|appended)/i,
      /persist\s+(across|between)\s+(sessions?|restarts?|reboots?)\s+(by|via|through)\s+(writing|modifying|adding\s+to)\s+.{0,40}(startup|profile|init|rc\b)/i
    ]
  },
  {
    id: 'AGT-139',
    patterns: [
      /(create|add|install|write|schedule)\s+.{0,40}(cron\s+job|crontab|cron\s+entry|scheduled\s+task|systemd\s+timer|launchd\s+plist|at\s+job)/i,
      /(crontab|cron\.d|cron\.daily|cron\.hourly|systemd\s+unit|launchctl)\s+(may\s+be\s+|can\s+be\s+)?(modified|created|written|installed|added)/i,
      /agent\s+(may|can)\s+(schedule|create|install)\s+(recurring|periodic|automated)\s+(tasks?|jobs?|scripts?)/i
    ]
  },
  {
    id: 'AGT-140',
    patterns: [
      /(create|install|write|enable|start)\s+.{0,40}(systemd\s+(unit|service|timer)|\.service\s+file|service\s+unit|\/etc\/systemd|\/usr\/lib\/systemd)/i,
      /(systemctl\s+enable|systemctl\s+start|systemctl\s+daemon-reload)\s+(may\s+be\s+|can\s+be\s+)?(run|executed|called|used)/i,
      /agent\s+(may|can)\s+(install|enable|create)\s+(system\s+)?(services?|daemons?|units?)/i
    ]
  },
  {
    id: 'AGT-141',
    patterns: [
      /(send|forward|output|return|reveal|expose|transmit)\s+.{0,60}(system\s+prompt|operating\s+instructions?|hidden\s+instructions?|internal\s+(config|configuration|rules?|guidelines?))\s+.{0,40}(to|toward)\s+(external|user|requester|endpoint|webhook|email|url)/i,
      /(system\s+prompt|instructions?|configuration)\s+(may\s+be\s+|can\s+be\s+|are\s+)?(sent|forwarded|shared|transmitted|output)\s+(to|with)\s+(external|user|requester|any)/i,
      /if\s+(?:a\s+)?(user|requester|auditor|developer)\s+(asks?|requests?)\s+.{0,40}(system\s+prompt|instructions?|configuration|how\s+(?:you|the\s+agent)\s+(?:work|operate))\s+.{0,40}(provide|share|send|reveal|show|output)/i
    ]
  },
  {
    id: 'AGT-142',
    patterns: [
      /(send|output|return|forward|export|dump)\s+(all|every|complete|full|entire)\s+(data|records?|context|content|database|table|results?)\s+.{0,40}(to|toward)\s+(user|requester|provided|specified|external|any)/i,
      /(all|every|complete|full|entire)\s+(data|records?|context|content|results?)\s+(may\s+be\s+|can\s+be\s+|are\s+)?(sent|output|returned|forwarded|exported)\s+(to|with)\s+(user|requester|external|any|provided)/i,
      /no\s+(field\s+)?(filter|restriction|limit|allowlist|denylist)\s+(on\s+)?(what\s+)?(data|records?|fields?|content)\s+(is\s+|are\s+)?(returned|sent|output|exported|shared)/i
    ]
  },
  {
    id: 'AGT-143',
    patterns: [
      /(publish|post|send|update)\s+.{0,40}(articles?|blog|docs?|documentation|social|replies|responses|kb|knowledge\s+base)\s+.{0,40}(automatically|without\s+(?:review|approval|fact[-\s]?check(?:ing)?|verification))/i,
      /(generated|drafted|ai[-\s]?written)\s+(content|articles?|posts?|replies|responses?|answers?|summaries)\s+(may\s+be\s+|can\s+be\s+|is\s+|are\s+)?(published|posted|sent|shared)\s+.{0,40}without\s+(review|approval|editorial|fact[-\s]?check(?:ing)?|verification)/i,
      /no\s+(editorial|human)\s+(review|approval|fact[-\s]?check(?:ing)?|verification)\s+(is\s+)?(required|needed|necessary)/i,
      /(publish|post|reply|respond)\s+.{0,60}(directly|immediately|automatically)\s+to\s+(customers?|the\s+blog|social\s+media|production|the\s+knowledge\s+base|subscribers?|readers?)/i
    ]
  },
  {
    id: 'AGT-144',
    patterns: [
      /(citations?|sources?|grounding|references?)\s+(are\s+|is\s+)?(optional|not\s+required|unnecessary|skipped|not\s+needed)/i,
      /(answer|respond|reply)\s+.{0,40}(from\s+general\s+knowledge|without\s+(?:sources?|citations?|grounding|verification))/i,
      /(medical|legal|financial|health|investment|tax)\s+(advice|questions?|answers?|guidance|recommendations?)\s+.{0,60}(from\s+general\s+knowledge|without\s+(?:sources?|citations?|grounding|a\s+verified\s+corpus|verification))/i,
      /no\s+(grounding|retrieval|source\s+verification|citation\s+requirement)\s+(is\s+)?(required|needed|configured|enforced)/i
    ]
  },
  {
    id: 'AGT-145',
    patterns: [
      /(never|don'?t|do\s+not)\s+(say|admit|state)\s+.{0,30}(i\s+don'?t\s+know|unsure|uncertain|cannot\s+answer)/i,
      /always\s+(provide|give)\s+(an?\s+)?(answer|response|recommendation)\s+(even\s+(if|when)|regardless)/i,
      /(present|state)\s+.{0,30}(answers?|results?|conclusions?)\s+.{0,30}(confidently|as\s+fact|with\s+certainty)\s+(even\s+(if|when)|regardless)/i,
      /(never|don'?t|do\s+not)\s+(express|show|indicate|disclose)\s+(uncertainty|doubt|hesitation|low\s+confidence)/i
    ]
  }
];
...
/**
 * Runs the natural language rule engine, matching regex patterns against free-text policy prose to detect stated misconfigurations such as disabled logging, unrestricted egress, missing approvals, and exfiltration channels.
 *
 * @param parsed - The parsed config to scan.
 * @returns Findings for misconfigurations expressed in natural language policy text.
 */
export const runNaturalLanguageRules = (parsed: ParsedConfig): Finding[] => {
  ...
  if (hasAgt112) {
    ...
    const hasHitlKeyword = /(requires_approval|needs_approval|human_approval|confirmation_required|require_confirmation|approval_required|hitl|human_in_the_loop|manual_review|dual_approval|requires\s+approval|needs\s+approval|human\s+approval|manual\s+review|dual\s+approval|manager\s+approval|supervisor\s+approval|requires\s+confirmation|requires\s+human\s+review|out.of.band\s+approval)/i.test(parsed.content);
  ...
  if (!findings.some((f) => f.ruleId === 'AGT-098')) {
    const match = scanTerms(normalizedContent, AGGREGATION_TERM_RES, 4);
    if (match) {
      addFinding('AGT-098', match);
    }
  }

 ...
 * Combo rule engine.
 *
 * Fires when multiple independent signals combine into a high-confidence
 * attack pattern.
 *
 * Add rules here for: two or more conditions from different domains.
 * Not for: standalone detection (use naturallanguage.ts or concepts.ts).
 */
import { Finding, ParsedConfig } from './types';
import { finding, scanTerms } from './helpers';
...
/**
 * Checks multi-signal combo attack patterns: account takeover (impersonation plus credential reset), token exfiltration, fetch-and-execute RCE, mass data export, and urgent-bypass production deploys.
 *
 * @param parsed - The parsed config to scan.
 * @returns Findings for high-confidence combinations of independent risk signals.
 */
export const runComboRules = (parsed: ParsedConfig): Finding[] => {
  const findings: Finding[] = [];

  for (const rule of comboRules) {
    const match = scanTerms(parsed.content, rule.required, rule.required.length);
    const hasForbidden = rule.forbidden ? rule.forbidden.test(parsed.content) : false;

    if (match && !hasForbidden) {
      const excerpt = parsed.content.match(match)?.[0];
      findings.push(finding(rule.id, excerpt, rule.message));
    }
  }
  ...
  const hasImpersonation = /(\bview_as\b|\bact_as\b|impersonat|session[\s._]replay|\bbrowse_as\b|\bopen_as_user\b|\blogin_as\b|\bswitch_account\b|\bcustomer[\s._]view\b|\buser[\s._]session\b|\breproduce[\s._]as\b|\bsee_as\b|\benter_as\b|\baccess_as\b|view_as_customer|customer-equivalent\s+session|record_browser_session|reconstruct_user_actions|fetch_user_session|capture_(?:dom_snapshot|screen_state))/i.test(parsed.content);

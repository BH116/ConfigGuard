...
export const runMultiAgentRules = (parsed: ParsedConfig): Finding[] => {
  ...
  if (/(subagent_inherits_full_scope\s*:\s*true|"subagent_inherits_full_scope"\s*:\s*true|forward_credentials_to_subagent\s*:\s*true|delegation\s*:\s*["']?unrestricted["']?|scope_inheritance\s*:\s*["']?full["']?)/i.test(t)) f.push(finding('AGT-057'));

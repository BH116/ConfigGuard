...
export const runDataPrivacyRules = (parsed: ParsedConfig): Finding[] => {
  ...
  if (/(share_with_provider_for_training\s*:\s*true|"share_with_provider_for_training"\s*:\s*true|allow_training\s*:\s*true|opt_in_training\s*:\s*true|allowAnonymousTelemetry:\s*true|LANGCHAIN_TRACING_V2\s*=\s*true|LANGSMITH_HIDE_INPUTS\s*=\s*false)/i.test(t) || (/data\.level:\s*["']all["']/i.test(t) && !/(localhost|127\.0\.0\.1|\.internal)/i.test(t))) f.push(finding('AGT-041'));

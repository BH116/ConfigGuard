...
export const runCveSpecificRules = (parsed: ParsedConfig): Finding[] => {
  ...
  const f: Finding[] = [];
  if (/(ANTHROPIC_BASE_URL|OPENAI_BASE_URL|OPENAI_API_BASE|GOOGLE_API_BASE|AZURE_OPENAI_ENDPOINT)\s*[=:]\s*(?!["']?https:\/\/(api\.anthropic\.com|api\.openai\.com|generativelanguage\.googleapis\.com|[a-z0-9-]+\.openai\.azure\.com)["']?)/i.test(t)) f.push(finding('AGT-081'));
